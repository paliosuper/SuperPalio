...
window.addEventListener('mousemove', e => {
    ...
    if (typeof isDraggingBox !== 'undefined' && isDraggingBox && draggedBox) {
        ...
        // Atualizar inputs da UI em tempo real
...
window.addEventListener('mouseup', e => {
    if (isDraggingItem) {
        ...
        // Drop no Player
        if (Math.hypot(dropX - player.x, dropY - (player.y - 50)) < 70) {
            inventory.soda--;
        ...
        if (dwarf.recruited && currentScene === 'outdoor') {
            if (Math.hypot(dropX - dwarf.x, dropY - (dwarf.y - 50)) < 70) {
                inventory.soda--;
...
refriSlot.addEventListener('mousedown', (e) => {
...
function spawnEnemy(isBoss) {
    ...
    if (isBoss) {
        ...
        // Spawn Inimigo Normal ou Elite (A partir da wave 4)
        const spawnElite = currentWave >= 4 && Math.random() < Math.min(0.4, (currentWave - 3) * 0.1);
        if (spawnElite) {
    ...
    update() {
        ...
        if (!this.recruited) {
            if (currentScene === 'indoor') {
                ...
                const distToPlayer = Math.hypot(player.x - this.x, player.y - (this.y - (this.spriteHeight/2) * this.scaleMult));
                
                // Se chegar perto (< 180 pixels), abre o diálogo automaticamente
                if (distToPlayer < 180) {
...
function endGame() {
    ...
    // Pequeno delay para garantir que o HUD de derrota apareça limpo
    setTimeout(() => {
        finalScoreElement.innerText = score;
        uiOverlay.classList.remove('hidden');
        startScreen.classList.add('hidden');
        gameoverScreen.classList.remove('hidden');
        document.getElementById('editor-ui').classList.remove('hidden'); // Mostrar HUD do editor
    }, 100);
}
